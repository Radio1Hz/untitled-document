...
The inverse of the logistic map of $f(x)$ is given by:
$$
...
It maps values on y axis back to x.
...
Sequence starts at an arbitrary value of $x = x_0$, where

- $x_0 \in [0,1]$
- $r \in [0,4]$

Applying the Logistic Map on $x_0$ we get the value $x_1$ on y-axis.
...
Applying $x_2$ on x-axis to Logistic Map, we get new value $x_3$ on y-axis.
...
Applying $x_4$ on x-axis to Logistic Map, we get new value $x_5$ on y-axis.

![Iteration 5](images/iteration-5.jpg)

The sequence continues in the same manner - "bouncing" from the direct to the inverse Logistic Map.
...
$$
This approach provides a 2D perspective on the dynamics of the logistic map.
...
For certain values of $r$ (denoted as $r_o3$), the iteration sequence stabilizes into a three-node cycle. To calculate $r_o$, we need to solve the equations 1 and 2:
